// ======================
// 🔠 Quiz de Siglas Médicas (Modo Rápido)
...
export async function initGame({ root, saveLocalScore, getAlias, updateBestBadge }) {
  // ---------- Config ----------
  const GAME_ID = "siglas";
  const ROUNDS = 10;
  const TIME_PER_Q = 15000; // 15 segundos
  const DATA_URL = "./data/siglas.json";

  // ---------- Estado (Global al módulo) ----------
  const st = {
    i: 0, score: 0, streak: 0,
    questionPool: [],
    review: [], // Para revisión de fallos
  ...
  function buildGameUI() {
    ...
    const nav = el("div", { class: "qz-meta", id: "qzNav", style: "display:none" }); // Vacío
  ...
  function showStartScreen() {
    ...
    $("#stem").innerHTML = `Juego de <strong>Siglas Médicas</strong>. Tienes ${TIME_PER_Q / 1000} segundos por sigla.<br>Pulsa 'Comenzar' cuando estés listo.`;
    $("#opts").append(
      el("button", {
  ...
  function timeOver() {
    ...
    st.review.push({
      pregunta: current.pregunta, // La sigla
  ...
  function reveal(idx, ok, elapsed) {
    ...
    if (ok) {
      ...
      st.streak++;
      const base = 10;
      const speed = Math.max(0, 5 - Math.floor(elapsed / 2000)); // Bonus
      st.score += base + speed;
    } else {
      st.streak = 0;
    }
    ...
    // No mostramos explicación intermedia
    
    st.i++;
    setTimeout(() => nextQ(), 600); // Auto-avance
  }
  ...
  function nextQ() {
    ...
    $("#stem").innerHTML = `<strong>Sigla:</strong> <span class="sigla-stem">${current.pregunta}</span>`;
  ...
  function endGame() {
    ...
    saveLocalScore(GAME_ID, st.score);
  ...
  function injectLiteStyles() {
    // Reutiliza los estilos del quiz-dx
    if (document.getElementById("quizdx-lite-css")) {
      if (document.getElementById("siglas-css")) return;
      const cssSiglas = `.sigla-stem { font-size: 1.5rem; font-weight: 700; color: #003a6b; }`;
      const style = document.createElement("style");
      style.id = "siglas-css";
      style.textContent = cssSiglas;
      document.head.append(style);
      return;
    }
    
    // Si 'quizdx-lite-css' no existe, lo crea todo
    const css = `
      ...
      .sigla-stem { font-size: 1.5rem; font-weight: 700; color: #003a6b; } /* Específico */
      ...
      /* Estilos para la revisión de errores */
  ...
  try {
    ...
    BANK = await res.json();
    BANK = BANK.filter(q => q && q.pregunta && Array.isArray(q.opciones) && typeof q.correcta === "number");
    if (BANK.length < 10) throw new Error("Banco de datos insuficiente (necesita > 10).");
