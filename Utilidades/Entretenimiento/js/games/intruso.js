...
export async function initGame({ root, saveLocalScore, getAlias, updateBestBadge }) {
  // ---------- Config ----------
  const GAME_ID = "intruso";
  const ROUNDS = 10;
  const TIME_PER_Q = 25000; // 25 segundos
  const DATA_URL = "./data/intruso.json";
  ...
  function showStartScreen() {
    ...
    $("#stem").innerHTML = `Juego de <strong>El Intruso</strong>. Tienes ${TIME_PER_Q / 1000} segundos por pregunta.<br>Pulsa 'Comenzar' cuando estés listo.`;
    $("#opts").append(
      el("button", {
  ...
  function timeOver() {
    ...
    st.review.push({
      pregunta: current.pregunta, // La categoría
  ...
  function reveal(idx, ok, elapsed) {
    ...
    if (ok) {
      ...
      st.streak++;
      const base = 10;
      const speed = Math.max(0, 5 - Math.floor(elapsed / 2000));
      st.score += base + speed;
    } else {
      st.streak = 0;
    }
  ...
  function nextQ() {
    ...
    $("#stem").innerHTML = `<strong>${current.pregunta}</strong> (Elige el intruso)`;
