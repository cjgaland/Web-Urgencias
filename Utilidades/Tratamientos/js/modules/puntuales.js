...
export const PuntualesModule = (() => {
  ...
  async function init() {
    if (!Array.isArray(GlobalState.puntuales)) StateManager.updateState({...GlobalState, puntuales:[]});
    try { VIA_OPCIONES = await cargarVias(); } catch(e){ VIA_OPCIONES=[]; }
  ...
  function addPuntual() {
    ...
    StateActions.addToArray('puntuales', { farmaco, dosis, via, indicacion, cuando, obs, timestamp: new Date().toISOString() });
    limpiarFormulario();
    renderTable();
  ...
  function updatePuntual() {
    ...
    const original = GlobalState.puntuales[moduleState.editingIndex] || {};
    const item = { farmaco, dosis, via, indicacion, cuando, obs, timestamp: original.timestamp || new Date().toISOString() };
  ...
  function renderTable() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '';
    const lista = (GlobalState.puntuales || []).slice().reverse();

    if (lista.length === 0) {
      tbody.innerHTML = `<tr style="text-align:center;color:#64748b"><td colspan="8">No hay medicación puntual</td></tr>`;
      return;
    }

    lista.forEach((it, i) => {
      const realIndex = (GlobalState.puntuales.length - 1) - i;
      ...
      tr.innerHTML = `
        ...
        <td>
          <div style="display:flex;gap:4px;justify-content:center">
            <button class="btn btn-sm edit" data-action="edit-puntual" data-index="${realIndex}"><i class="fas fa-edit"></i></button>
            <button class="btn btn-sm danger" data-action="delete-puntual" data-index="${realIndex}"><i class="fas fa-trash"></i></button>
          </div>
        </td>
      `;
  ...
  function limpiarFormulario() {
    ...
    DOMHelpers.setValue('#pun_cuando', 'ahora');
  ...
  StateManager.subscribe((nuevo, anterior, cambios) => {
    if (cambios.puntuales !== undefined) renderTable();
  });

  return { init, addPuntual, updatePuntual, cancelarEdicion, vaciar, add: addPuntual, guardar: updatePuntual, clear: vaciar };
})();
